...
class BuyResourceRxSpec: QuickSpec, TradeResourceRxSpecProtocol {
    
    override func spec() {
        let mockedSessionManager = specVar { MockedSessionManager() }
        
        let type = TradeResourceType.buys
        let resource = specVar { Coinbase(sessionManager: mockedSessionManager()).buyResource }
